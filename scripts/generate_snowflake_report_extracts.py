...
QUERY_EXPORTS = [
    (
        ...
        "Supporting context-engine summary used to verify value, cost, stability, and review volume in Snowflake.",
    ),
    (
        "comp_mix",
        "MARTS.VW_COMP_MIX",
    ...
    (
        "policy_decision_recommendation",
        "MARTS.VW_POLICY_DECISION_RECOMMENDATION",
        "Selected shadow-validation policy and the exact metrics supporting the executive recommendation.",
    ),
    (
        "policy_tradeoff",
        "MARTS.VW_POLICY_TRADEOFF",
        "Candidate-policy adequacy, cost, refund, manager-review, and robustness tradeoffs.",
    ),
    (
        "policy_segment_diagnostics",
        "MARTS.VW_POLICY_SEGMENT_DIAGNOSTICS",
        "Unsuppressed segment-level diagnostics for policy safety review.",
    ),
    (
        "policy_uncertainty",
        "MARTS.VW_POLICY_UNCERTAINTY",
        "Probabilistic guardrail and modeled cost uncertainty by policy.",
    ),
]
...
def main() -> int:
    ...
    write_json(
        SNOWFLAKE_EXTRACT_MANIFEST_PATH,
        {
            ...
            "source": "Snowflake analytic views",
            "extracts": {
                result["name"]: {
