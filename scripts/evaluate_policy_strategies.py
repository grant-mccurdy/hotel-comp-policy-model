...
def rank_simple_strategy(
    ...
    if strategy == "lowest_cost_adequate":
        sensitivity_low = as_float(load_policy_scenarios()["probabilistic_sensitivity"]["multiplier_low"], 0.8)
...
def select_simple_strategy(
    ...
    return rank_simple_strategy(strategy, row, tier, catalog, policy, evaluation)[0]


def recommend_policy_strategy(mapping: dict[str, Any], policy_id: str) -> dict[str, Any]:
...
def probabilistic_sensitivity(
    ...
    evaluation = scenario_config["evaluation"]
    guardrails = scenario_config["pilot_guardrails"]
...
def build_summary_rows(
    ...
    selected_id, recommendation = select_pilot_policy(metrics_by_policy, uncertainty_rows, scenario_config)
        ...
        rows.append(
            {
                ...
                "selected_for_pilot": str(policy_id == selected_id).lower(),
...
def render_report(
    ...
    for row in summary_rows:
        ...
        lines.append(
            ...
            f"{'Shadow-validation candidate' if truth(row['selected_for_pilot']) else 'Comparator'} |"
        )
    lines.extend(
        [
            "",
