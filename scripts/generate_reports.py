...
def canonical_worked_decision() -> dict[str, object]:
    scenario = {
    ...
    return build_decision(scenario, bundle).as_dict()
...
def render_executive_brief(
    ...
    example = canonical_worked_decision()
    example_recommendation = dict(example["recommendation"])
    example_confidence = dict(example["confidence"])
    example_approval = dict(example["approval"])
    example_alternative = dict(list(example["alternatives"])[0])
    ...
    for row in summary_rows:
        ...
        marker = " **(shadow-validation candidate)**" if row.get("selected_for_shadow_evaluation") == "true" else ""
    ...
    lines = [
        ...
        *comparison_lines,
        ...
        "",
        f"**Recommended recovery:** {example_recommendation['comp_label']} + personal manager note",
        "",
        f"- Modeled guest-facing value: `{money(example_recommendation['guest_facing_value'])}`",
        f"- Assumed internal-cost range: `{money(example_recommendation['internal_cost_low'])}-{money(example_recommendation['internal_cost_high'])}`",
        f"- Approval path: `{example_approval['approval_path']}`",
        f"- Input-sensitivity stability: `{as_float(example_confidence['input_sensitivity_stability']):.0%}` ({example_confidence['level']})",
        f"- Closest alternative: `{money(example_alternative['guest_facing_value'])} {example_alternative['comp_label']}`",
...
def render_dashboard(decision: dict[str, object], case_rows: list[dict[str, str]]) -> str:
    ...
    comparison_rows = "".join(
        ("<tr class=\"selected\">" if row.get("selected_for_shadow_evaluation") == "true" else "<tr>")
...
def render_discussion_brief(
    ...
    example = canonical_worked_decision()
    recommendation = dict(example["recommendation"])
    approval = dict(example["approval"])
    return "\n".join(
        [
            ...
            "",
            f"**Example:** {recommendation['comp_label']} + personal manager note, with {money(recommendation['guest_facing_value'])} modeled guest-facing value and an assumed internal-cost range of {money(recommendation['internal_cost_low'])}-{money(recommendation['internal_cost_high'])}.",
            "",
            f"The example follows the `{decision['selected_policy_label']}` candidate and uses the `{approval['approval_path']}` path.",
            "",
